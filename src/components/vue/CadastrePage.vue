<template>
    <div class="w-full flex flex-col items-center bg-primary-800 py-10 custom-bg">
        <div class="px-container w-full">
            <Breadcrumb :current-page="currentPage" class="border-none p-0 !bg-transparent" />
        </div>
        <div class="w-full px-container flex flex-col gap-4 md:gap-6 pt-8 pb-16 items-center justify-center">
            ...
            <div class="w-full rounded-2xl p-8 max-w-3xl border bg-white">
                <form @submit.prevent="submitForm" class="flex flex-col gap-4 md:gap-8">
                        ...
                        <div class="flex flex-col gap-4">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-0 m-0">
                                <div class="flex flex-col gap-1.5">
                                    ...
                                    <Select v-model="formData.finalidade" :options="finalidades" class="w-full p-variant-secondary" />
                                </div>
                                <div class="flex flex-col gap-1.5">
                                    <label class="pl-2">Escolha o tipo de imóvel</label>
                                    <Select v-model="formData.tipo" :options="tiposImovel" class="w-full p-variant-secondary" />
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 p-0 m-0">
                                <div class="flex flex-col gap-1.5">
                                    <label class="pl-2">Digite a cidade</label>
                                    <InputText v-model="formData.cidade" class="w-full" />
                                </div>
                                <div class="flex flex-col gap-1.5">
                                    <label class="pl-2">Digite o bairro</label>
                                    <InputText v-model="formData.bairro" class="w-full" />
                                </div>
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="pl-2">Digite o valor pretendido</label>
                                <InputNumber v-model="formData.valor" mode="currency" currency="BRL" locale="pt-BR" class="w-full" />
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="pl-2">Digite sua mensagem</label>
                                <Textarea v-model="formData.mensagem" rows="5" class="w-full" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="align-items-center gap-2">
                        <Checkbox v-model="formData.aceitaPolitica" binary inputId="acceptPrivacy" />
                        <label for="acceptPrivacy" class="ml-2 text-sm cursor-pointer">
                            Declaro estar ciente que a ação de envio deste formulário permite que seja contatado pela Imobiliária Rio Branco, assim como estar de acordo com o exposto na 
                            <a href="/politica-de-privacidade" class="text-primary-500 hover:underline" target="_blank">política de cookies</a>.
                        </label>
                    </div>

                    <!-- Botão de Envio -->
                    <div class="flex justify-end w-full mt-4 md:mt-0">
                        <Button type="submit" label="Cadastrar Imóvel" rounded="full" severity="primary" :disabled="!formData.aceitaPolitica" class="w-full md:w-fit" />
                    </div>
                </form>
            </div>
            ...
            <div class="w-full max-w-3xl flex flex-col gap-8 mt-12">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                    <div v-for="(servico, index) in servicos" :key="index" 
                        ...
                        <i :class="servico.icon" class="text-2xl"></i>
