<template>
	<div class="flex flex-col md:flex-row w-full gap-4 md:gap-2">
		<Panel ref="panel" :toggleable="true" :collapsed="true" @toggle="onToggle" class="w-full" :pt="{
			root: {
			...
			<template #header>
				<div class="flex flex-col md:flex-row md:justify-between w-full items-center gap-2 py-1.5 md:py-0.5">
					<div class="flex w-full">
						<Select optionLabel="name" optionValue="code" :options="options" class="w-fit hidden md:flex"
					...
					<div class="flex items-center gap-2 md:w-fit w-full">
						<Select optionLabel="name" optionValue="code" :options="options" class="w-fit flex md:hidden"
							v-model:model-value="selectedOption" />
						<Button :icon="expanded ? 'pi pi-minus' : 'pi pi-plus'" :label="expanded ? 'Filtros' : 'Filtros'"
							class="rounded-full w-full md:w-fit" outlined @click.prevent="togglePanel" />
					</div>
				</div>
			</template>
			<div class="flex flex-wrap gap-2">
				<Select placeholder="Tipo de Negócio" :options="businessTypes" optionLabel="name" optionValue="code"
					class="w-auto p-variant-secondary" />
				<Select placeholder="Tipo de Imóvel" :options="propertyTypes" optionLabel="name" optionValue="code"
					class="w-auto p-variant-secondary" />
				<Select placeholder="Cidade" :options="cities" optionLabel="name" optionValue="code"
					class="w-auto p-variant-secondary" />
				<Select placeholder="Detalhes" optionLabel="name"
					class="w-auto p-variant-secondary" @click="showExtraDetails = !showExtraDetails" />
				<div v-if="showExtraDetails" class="flex flex-wrap gap-2 w-full mt-2">
					<div class="flex flex-col gap-1">
						<label class="pl-2">Dormitórios</label>
						<Slider v-model="bedrooms" :min="1" :max="10" class="w-24" />
					</div>
					<div class="flex flex-col gap-1">
						<label class="pl-2">Vagas para carro</label>
						<Slider v-model="parkingSpots" :min="1" :max="6" class="w-24" />
					</div>
					<div class="flex flex-col gap-1">
						<label class="pl-2">Valor do imóvel</label>
						<Slider v-model="priceRange" :min="0" :max="maxValue" class="w-64" />
					</div>
				</div>
			</div>
		</Panel>

		<Button rounded as="a" :href="searchLink" label="Buscar" severity="secondary" icon="pi pi-search" rounded-full
